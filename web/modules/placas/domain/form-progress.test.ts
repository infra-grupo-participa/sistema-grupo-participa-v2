...
import { isPlateEligible, isTokenDocumentUrl, validateFormProgress } from './form-progress';
...
describe('form-progress — validação progressiva', () => {
  ...
  it('eligible não pode ser cadastro_concluido', () => {
    expect(
      validateFormProgress({ token: TOKEN, step_index: 3, nivel: 'ouro', faturamento_declarado: 1, ...step1, interesse: 'x', espaco_instrucao: 'casa', status: 'cadastro_concluido' })?.code,
    ).toBe('invalid_eligible_status');
  });

  it('step 4/5 exigem documentos válidos do token', () => {
    const base = { token: TOKEN, ...step1, interesse: 'x', espaco_instrucao: 'casa', nivel: 'ouro', faturamento_declarado: 1 };
