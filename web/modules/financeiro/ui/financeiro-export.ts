...
const HEADERS = [
  ...
  'Vencimento',
  'Dias em atraso',
  // O ACORDO comercial e seus termos — o financeiro cobra em cima do que foi
  // combinado, então a planilha precisa levar o combinado junto com a dívida.
  'Acordo',
  'Meio combinado',
  'Forma',
  'Parcelas pagas',
  ...
  'Valor da parcela',
  // Promessa remarcada N vezes = promessa quebrada. É o sinal de risco do financeiro.
  'Remarcações da promessa',
  'Cobranças feitas',
  'Última cobrança',
  // Onde a pessoa está na ativação — leitura rasa, para saber com quem se fala.
  'Situação na ativação',
  'Status',
  'Documento',
  'Turma',
  'Pacote',
...
function buildRows(rows: ContaReceber[], canVerDoc: boolean): (string | number)[][] {
  return rows.map((c) => [
    ...
    c.vendedor ?? '',
    c.reuniao_resultado ?? '',
    c.solicitou_cancelamento ? 'Sim' : 'Não',
    c.sinal_bruto ?? '',
    c.sinal_liquido ?? '',
    ...
    c.remarcacoes ?? 0,
    c.cobrancas_total ?? 0,
    c.ultima_cobranca_em ? fmtData(c.ultima_cobranca_em) : '',
    c.estagio_nome ?? '',
